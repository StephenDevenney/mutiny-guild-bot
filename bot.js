    ...
    function reload() {
        ...
        if(eventList.length > 0){
            eventList.forEach(item => {
                ...
                let dateTime = date + "T" + item.eventTime;
...
client.on('message', message => {
    ...
    if(!message.member.roles.cache.some(role => role.name === "Officer" || role.name === "O' Captain My Captain")) return message.channel.send("Unauthorized user"); 

    if(message.channel.name != process.env.CHANNEL_OFFICER) return;

    // Check Shorthand
    let shorthand = false;
    if(message.content.startsWith(process.env.PREFIX_OFFICER) || message.content.startsWith(process.env.PREFIXSHORT_OFFICER)){
        if(message.content.startsWith(process.env.PREFIXSHORT_OFFICER))
            shorthand = true;
    }
    else
        return;
    ...
    let prefix = "";
    if(shorthand){
        removePrefix = message.content.slice(process.env.PREFIXSHORT_OFFICER.length);
        prefix = process.env.PREFIXSHORT_OFFICER;
    } 
    else{
        removePrefix = message.content.slice(process.env.PREFIX_OFFICER.length);
        prefix = process.env.PREFIX_OFFICER;
    } 

    const args = removePrefix.slice(prefix).trim().split(/ +/g);
    ...
    switch(command){   
        ...
        try {
            ...
            // }
            // catch(err){
            //     messageFilter("Incorrect value type entered, Please enter in a number.");
            //     return;
            // }
            ...
            //     return;
            // }
            // argsPoints = args[args.length-1];
            // activityPoints = args[args.length-1];
            ...
            //     userObj = item;
            //     givenPoints = 0;
            //     user = client.findUser.get(userObj.username);        
            //     activityPoints = activityCmds.addPoints(argsPoints, user.points);
        ...
        case 'f':
            try{
                ...
                messageFilter(findCmds.embededFindMessage(user), discU.avatarURL);
                break;
            }
            catch(err){
        ...
        case 'del':
            try{
                ...
                itemName = args.slice(1, (args.length-2));
    ...
    function guidesChannel(){
        ...
        // const args = removePrefix.slice(process.env.PREFIX_ACTIVITY).trim().split(/ +/g);
        // const command = args.shift().toLowerCase();

        // switch(command){   
        //     case 'khan': 
        //         try {
        //             messageFilter("https://preview.redd.it/fx5or768at341.png?auto=webp&s=fcd8f0893e9ad07fec1681504f36d6d684e40619");
        //             break;
        //         } 
        //         catch (err){
        ...
        //     case 'Tribute Wagon':
        //         try {
        //             messageFilter("https://www.youtube.com/watch?v=HH3viEDNHjE");
        //             break;
        //         } 
        //         catch (err){
        //             let errMessage = helpCmds.errorMessage("Error retrieving guide", err, message);
        ...
        //     case 'Peridot Leaves':
        //         try {
        //             messageFilter("https://i.imgur.com/DqLcqQd.jpg");
        //             break;
        //         } 
        //         catch (err){
        //             let errMessage = helpCmds.errorMessage("Error retrieving guide", err, message);
        ...
        //         }   
        //     default:  
        //         messageFilter("No guide available");   
        // }
    ...
    function guidesChannel(){
        let shorthand = false;
        if(message.content.startsWith(process.env.PREFIX_PUBLIC) || message.content.startsWith(process.env.PREFIXSHORT_PUBLIC) || message.content.startsWith(process.env.PREFIX_GUIDES)){
            if(message.content.startsWith(process.env.PREFIX_GUIDES) || message.content.startsWith(process.env.PREFIXSHORT_PUBLIC))
                shorthand = true;
        }
        else
            return;
    ...
    function gamesRoom(){
        let shorthand = false;
        if(message.content.startsWith(process.env.PREFIX_PUBLIC) || message.content.startsWith(process.env.PREFIXSHORT_PUBLIC)){
            if(message.content.startsWith(process.env.PREFIXSHORT_PUBLIC))
                shorthand = true;
        }
        else
            return;
        ...
        let prefix = "";
        if(shorthand){
            removePrefix = message.content.slice(process.env.PREFIXSHORT_PUBLIC.length);
            prefix = process.env.PREFIXSHORT_PUBLIC;
        } 
        else{
            removePrefix = message.content.slice(process.env.PREFIX_PUBLIC.length);
            prefix = process.env.PREFIX_PUBLIC;
        } 
    ...
    function messageFilter(msg){
        if(msg != "do not send")
            ...
            message.channel.send(helpCmds.help); 
                ...
                message.channel.send(helpCmds.syntaxHelp(command)); 
    ...
    function messageModerator(errMessage){
        ...
        if(errMessage != "Fatal error! moderator has been notified.")
            ...
            moderator.send(helpCmds.errorMessage("Error in activity points", "Newly added '" + command + "' command wasn't added to activityCmds.embededActivityMessage", message));
    }
