const DiscordF = require('discord.js');
...
exports.listUsersMessage = function(users){
    if(users.length > 0){
        let exampleEmbed = new DiscordF.MessageEmbed()
                .setColor(3447003)
        ...
        users.forEach(element => { 
            ...
            if(element.lastActivity != null)
                lastActivityDate = new Date(element.lastActivity).toString();    
...
exports.embededFindMessage = function(user, avatar){
    ...
    if(user.lastActivity != "")
        lastActivityDate = new Date(user.lastActivity).toString();
